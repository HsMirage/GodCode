...
test.describe('Settings Configuration', () => {
  ...
  test('GodCode branding visible in top nav', async ({ window }) => {
  ...
  test('newly added model hides protocol selector and stores no manual OpenAI protocol override', async ({
    window
  }) => {
    await navigateTo(window, 'settings')
    ...
    await window
      .locator('input[placeholder="例如: gpt-4, claude-3-opus..."]')
      .fill(initialModelName)
    await window.locator('button:has-text("添加模型")').first().click()
    ...
    const providerCard = window
      ...
      .first()
    await expect(providerCard).toBeVisible()
    await expect(window.locator('text=OpenAI 协议')).toHaveCount(0)

    const readProtocol = async () =>
      window.evaluate(async (targetModelName: string) => {
        const api = (
          window as unknown as {
            godcode?: { invoke: (channel: string, ...args: unknown[]) => Promise<unknown> }
          }
        ).godcode
        if (!api) return null
    ...
    await expect.poll(readProtocol).toBe(null)
