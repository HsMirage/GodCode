...
export function AgentCard({ agent, models, providerNameByModelId, onUpdate, onReset }: AgentCardProps) {
  ...
  const selectedModel = agent.modelId ? models.find(m => m.id === agent.modelId) : null
    ...
    >
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-medium text-[var(--text-primary)]">{agent.agentName}</h3>
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-[var(--text-secondary)] bg-[var(--bg-tertiary)] px-2 py-1 rounded border border-[var(--border-primary)]">
            {selectedModel ? `${selectedModel.modelName} (${selectedProvider})` : agent.modelName || '默认模型'}
          </span>
        ...
        <div className="px-4 pb-4 pt-2 border-t ui-border space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <select
              ...
              disabled={loading}
              className="w-full rounded-lg border border-[var(--border-primary)] bg-[var(--bg-primary)] px-3 py-2 text-sm text-[var(--text-primary)] focus:border-indigo-500/50 focus:outline-none"
            >
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              disabled={loading}
              className="w-full h-2 bg-[var(--bg-tertiary)] rounded-lg appearance-none cursor-pointer accent-indigo-500"
            />
          </div>
          ...
          {agent.tools.length > 0 && (
            <div className="space-y-1.5">
              <label className="text-xs text-[var(--text-secondary)] font-medium">可用工具</label>
              <div className="flex flex-wrap gap-1.5">
                {agent.tools.map(tool => (
                  <span
                    key={tool}
                    className="text-xs bg-[var(--bg-tertiary)] text-[var(--text-secondary)] px-2 py-0.5 rounded border border-[var(--border-primary)]"
                  >
                    {tool}
                  </span>
                ))}
            ...
            {showSystemPrompt ? (
              <div className="space-y-2">
                <textarea
                  ...
                  disabled={loading}
                  rows={6}
                  className="w-full rounded-lg border border-[var(--border-primary)] bg-[var(--bg-primary)] px-3 py-2 text-sm text-[var(--text-primary)] focus:border-indigo-500/50 focus:outline-none resize-none font-mono placeholder:text-[var(--text-muted)]"
                />
