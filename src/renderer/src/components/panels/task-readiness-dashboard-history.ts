...
const STORAGE_KEY = 'codeall.task-readiness.dashboard.history'

function readRecoveryMetadata(task: Task) {
  const metadata = task.metadata && typeof task.metadata === 'object'
    ? (task.metadata as Record<string, unknown>)
    : null
  const recovery = metadata?.recovery && typeof metadata.recovery === 'object'
    ? (metadata.recovery as Record<string, unknown>)
    : null

  return {
    recoverySource: typeof (recovery?.recoverySource || metadata?.recoverySource) === 'string'
      ? String(recovery?.recoverySource || metadata?.recoverySource)
      : null,
    recoveryStage: typeof (recovery?.recoveryStage || metadata?.recoveryStage) === 'string'
      ? String(recovery?.recoveryStage || metadata?.recoveryStage)
      : null,
    resumeReason: typeof (recovery?.resumeReason || metadata?.resumeReason) === 'string'
...
export function buildLiveTaskReadinessDashboardSnapshot(input: {
  ...
  return buildTaskReadinessDashboardSnapshot({
    ...
    metricNotes: {
      ...
      approval_hit_rate: '当前按待处理审批请求与 pending_approval 任务估算，后续可接入审批审计全量数据。',
