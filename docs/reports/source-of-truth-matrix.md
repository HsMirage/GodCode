...
### 5.1 口径定义
- **完整移植**：以能力目标为单位，要求在 GodCode 中可独立运行、可验证、可回归；不要求与参考项目保持源码级 1:1 结构一致。
- **能力适配**：允许使用 GodCode 现有架构（Workforce/Delegate/Tool Registry/IPC）实现同等能力目标；若工具名、执行入口或交互路径不同，必须给出映射关系与证据位置。
