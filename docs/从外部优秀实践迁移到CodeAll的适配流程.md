# 从外部优秀实践迁移到 CodeAll 的适配流程

本文给出将外部优秀实践（如 opencode / omo 的命令、Hook、权限、编排思路）迁移到 CodeAll 的标准流程。
...
3. 先适配后扩展：先落在 CodeAll 既有抽象，再新增能力。
4. 全程可审计：关键决策可回放、可定位。

基线任务来源：
`/Users/mirage/AI/AiWork/CodeAll/docs/opencode-omo-best-practices-task-list.md`

## 2. 外部能力到 CodeAll 的映射

| 外部实践能力 | CodeAll 对应能力 | 核心文件 |
|---|---|---|
| Slash 命令触发 | Skill registry + command items | `/Users/mirage/AI/AiWork/CodeAll/src/main/services/skills/registry.ts` |
| 命令模板执行 | Skill Runtime Payload 组装 | `/Users/mirage/AI/AiWork/CodeAll/src/main/ipc/handlers/message.ts` |
| 工具名兼容 | Tool alias resolve | `/Users/mirage/AI/AiWork/CodeAll/src/main/services/tools/tool-registry.ts` |
| 执行权限模板 | PermissionPolicy + setting schema | `/Users/mirage/AI/AiWork/CodeAll/src/main/services/tools/permission-policy.ts` |
| 生命周期 Hook | HookManager + hooks index | `/Users/mirage/AI/AiWork/CodeAll/src/main/services/hooks/manager.ts` |
| Claude hooks 配置 | Claude Code adapter/config-loader | `/Users/mirage/AI/AiWork/CodeAll/src/main/services/hooks/claude-code/` |
| 工作流事件观测 | workforce events -> onTaskLifecycle | `/Users/mirage/AI/AiWork/CodeAll/src/main/services/workforce/events.ts` |
...
补充参考：
`/Users/mirage/AI/AiWork/CodeAll/docs/如何新增skill.md`

### 阶段 B：工具名兼容与作用域收敛

将外部工具名映射到 CodeAll 工具名：
...
文件：
`/Users/mirage/AI/AiWork/CodeAll/src/main/services/tools/tool-registry.ts`
`/Users/mirage/AI/AiWork/CodeAll/src/main/services/tools/tool-execution.service.ts`

补充参考：
`/Users/mirage/AI/AiWork/CodeAll/docs/工具alias配置规范.md`

### 阶段 C：权限模板适配

把外部“安全模式”迁移为 CodeAll 模板：
...
补充参考：
`/Users/mirage/AI/AiWork/CodeAll/docs/权限模板设计规范.md`
