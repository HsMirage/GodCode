...
- 建立 renderer 统一 API 分域封装：
  ...
  - `browserApi`
  - `settingsApi`
- 收敛页面直接 `window.codeall.invoke(...)` 的调用；
- 对关键 IPC 返回结构补显式类型定义。
...
从 `项目规划.md` 的视角看，CodeAll 当前已经过了“从 0 到 1”的阶段，下一步应进入“从可用到可持续”的阶段。
...
当以上五点完成后，CodeAll 才真正从“功能齐备的实验平台”迈向“可长期演进的产品级工作台”。
