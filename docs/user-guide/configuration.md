...
本文档详细介绍 CodeAll 的各项配置选项，包括 API Key 设置、模型配置、Agent 绑定等。
...
CodeAll 支持多个 LLM 提供商，需要配置相应的 API Key 才能使用。
...
| 平台 | 路径 |
|------|------|
| Windows | `%APPDATA%\CodeAll\` |
| macOS | `~/Library/Application Support/CodeAll/` |
| Linux | `~/.config/CodeAll/` |

### 数据库

CodeAll 使用嵌入式 PostgreSQL 存储数据:
